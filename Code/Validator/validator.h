...
ValidationResult validate(vector<ULD> &uldInfo, vector<PACKAGE> &packagesInfo,
                          Solution &sol, int &k) {
  ...
  vector<vector<vector<int>>> table(
    numUld, vector<vector<int>>(mxUldDim, vector<int>(mxUldDim, -1)));
  map<int, int> uldWeightSum;

  auto _packages = sol.getPackages();
  auto packages
    = vector<pair<int, AssignedPackage>>(_packages.begin(), _packages.end());

  sort(packages.begin(), packages.end(),
       [&](pair<int, AssignedPackage> left, pair<int, AssignedPackage> right) {
