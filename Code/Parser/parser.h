...
PACKAGE parsePackageInput(const string &inputRow) {
  ...
  string _packageIdentifier, _length, _width, _height, _weight, _priority,
    _delayCost;
  // Variables to hold the parsed data once typecasted to their respective data
  // type
  int packageIdentifier, length, width, height, weight, delayCost;
  bool priority = false;

  // Fetched the Identifier, Dimensions, Weight, Type(Priority/Economy), cost
  // of delay one at a time
  ...
  if(_priority[0] == 'P' || _priority[0] == 'p')
    priority = true;
  ...
  PACKAGE pkg(packageIdentifier, length, width, height, weight, delayCost,
              priority);
  return pkg;
}
...
void parseInput(int &uldPriorityPenalty, vector<PACKAGE> &packages,
                vector<ULD> &ulds, string inputPath) {
  ...
  while(getline(inputFile, inputRow)) {
    ...
    if(inputRow[0] == 'P' || inputRow[0] == 'p') {
      // Parsing the package details and populate the packages with the parsed
      // data
